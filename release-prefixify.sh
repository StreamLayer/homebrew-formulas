...
FORMULA_FILE="$SCRIPT_DIR/Formula/prefixify.rb"
...
git clone git@github.com:StreamLayer/prefixify.git prefixify || fail "Could not clone streamlayer/prefixify from github"

cd prefixify > /dev/null
...
if [ "$INSTALLED_VERSION" != "$VERSION" ]; then
  fail "The swift-prefixify version does not match the tag specified. Did you forget to update main.swift?"
fi
...
curl --show-error --verbose --fail -s --request PUT --user "avvs:${BINTRAY_API_KEY}" \
	...
	--header "X-Bintray-Package: Prefixify" \
	--header "X-Bintray-Version: ${VERSION}" \
	--header "X-Bintray-Publish: 1" \
	--upload-file "./prefixify--${RAW_VERSION}.catalina.bottle.tar.gz" \
	"https://api.bintray.com/content/streamlayer/bottles-formulas/prefixify-${RAW_VERSION}.catalina.bottle.tar.gz" \
	|| fail "Could not upload package to bintray"

output "Pushing updated formula"

git commit -a -m "Updated swift-prefixify formula" || fail "Could not commit formula"
git push || fail "Could not push formula to remote"

output "Testing whether install works"

brew uninstall prefixify
brew install streamlayer/formulas/prefixify || fail "Could not install swift-prefixify"
